...
#include <vector>
#include <string>
#include <opencv2/core/types.hpp>  // OpenCV Point2f 타입을 위해 필요
...
struct GolfBallInfo {
    cv::Point2f center;      // 골프공 중심 좌표
    float radius;            // 골프공 반지름
    float confidence;        // 검출 신뢰도 (0.0 ~ 1.0)
    int frameIndex;          // 프레임 인덱스
};

struct GolfBallDetectionResult {
    bool found;              // 골프공 발견 여부
    std::vector<GolfBallInfo> balls;  // 검출된 골프공들
...
public:
    struct DetectionParams {
        ...
        double param2 = 30.0;         // 원 검출 임계값
        int minRadius = 5;            // 최소 반지름 (픽셀)
        int maxRadius = 50;           // 최대 반지름 (픽셀)

        // 색상 필터링 파라미터
        int brightnessThreshold = 200;  // 밝기 임계값 (0-255)
        float minCircularity = 0.8f;    // 최소 원형도 (0.0-1.0)

        // 검출 옵션
        bool useColorFilter = true;     // 색상 필터 사용 여부
        bool useCircularityCheck = true;// 원형도 검사 사용 여부
        bool detectMultiple = false;    // 여러 개 검출 허용
    };
